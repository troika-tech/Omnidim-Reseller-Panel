...
const CallLogs = () => {
  ...
  // In-memory cache for recently fetched call logs
  ...
  // Note: Phone number logic is handled in the backend for user-specific filtering
  // The backend sends the correctly calculated displayPhoneNumber as 'phoneNumber'
  ...
  const fetchCallLogs = useCallback(async () => {
    try {
      ...
      // Serve cached paginated data when available (no call type filter)
      if (filters.call_type === "all") {
        ...
        const cacheKey = buildCacheKey("user-call-logs-page", filters, {
          page: pageno,
        });
        ...
        const response = await api.get("/user/call-logs", { params });

        if (response.data.success) {
          const currentData = response.data.data;
          const currentTotalPages = response.data.pagination?.pages || 1;
          const currentSync = response.data.sync || null;
      ...
      if (cachedFiltered && Date.now() - cachedFiltered.timestamp < CACHE_TTL) {
        ...
        setSyncStatus(cachedFiltered.syncStatus ?? null);
        setError(null);
        setIsDataLoading(false);
        setLoading(false);
        return;
      ...
      setSyncStatus(latestSyncStatus ?? null);
      setError(null);
      ...
      const fallbackMessage =
        ...
        (err.code === "ECONNABORTED"
          ? "Call log request timed out. Please try again."
          : err.message) ||
  ...
  // Simple loading logic - just check once
  useEffect(() => {
    const hasNoData = callLogs.length === 0;
    const isSyncInProgress = syncStatus?.inProgress;
    const hasRecentSync =
      syncStatus?.lastRunAt &&
      Date.now() - new Date(syncStatus.lastRunAt).getTime() < 300000; // 5 minutes

    // Show spinner if: no data AND sync activity AND haven't checked yet
    const shouldShow =
      hasNoData && !loading && (isSyncInProgress || hasRecentSync) && !hasCheckedOnce;
    
    setShouldShowSpinner(shouldShow);
    
    // If we should show spinner, do one check after 3 seconds
    if (shouldShow) {
      console.log("📡 Checking for call logs once...");
      const timeoutId = setTimeout(() => {
        console.log("🔄 Performing single check for call logs");
        fetchCallLogs();
  ...
  }, [callLogs, syncStatus, loading, hasCheckedOnce, fetchCallLogs]);
  ...
  const fetchStats = async () => {
    try {
      ...
      const response = await api.get("/user/call-logs/stats", { params });
  ...
  return (
    <div className="min-h-screen overflow-x-hidden">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs sm:text-sm lg:text-base text-gray-600 dark:text-gray-400 mt-1">
              View and analyze your assigned number call history
            </p>
            {syncStatus && (
              <div className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mt-2 space-y-1">
                <div>
                  {syncStatus.inProgress
                    ? "Syncing latest call logs in background…"
                    : syncStatus.lastRunAt
                    ? `Last synced ${formatSyncTimestamp(syncStatus.lastRunAt)}`
                    : "Call log sync status unavailable"}
                </div>
      ...
      <div className="mb-4 sm:mb-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-3 sm:gap-4">
        {/* Filter by Call Type */}
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-3 sm:p-4 lg:col-span-1">
            ...
            <div className="flex flex-col items-center gap-4">
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  {syncStatus?.inProgress
                    ? "Syncing latest call data in background"
                    : "Checking for call logs..."}
                </p>
                {syncStatus?.lastRunAt && (
                  <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
                    Last sync:{" "}
                    {new Date(syncStatus.lastRunAt).toLocaleTimeString()}
                  </p>
                )}
              </div>
            </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-cyan-600 dark:text-cyan-400">
                      {selectedCallLog.phoneNumber || "Unknown"}
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="text-gray-900 dark:text-white">
                      {selectedCallLog.campaignName || "Not found"}
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="text-blue-600 dark:text-blue-400">
                      {selectedCallLog.campaignId || "N/A"}
                    </p>
                  </div>
                </div>
